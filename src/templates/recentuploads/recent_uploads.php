...
<div class="container-fluid py-4 px-4">
    ...
    <div class="row mb-2 align-items-center">
        <div class="col-lg-8">
            <h6 class="text-theme-dark fw-bold mb-0" ng-click="navigateto('app.myfiles')" type="button"><i class="fa-solid fa-timer me-2"></i> Recent Uploads <span ng-show="folderdata"><i class="fa-solid fa-angle-right me-1"></i>{{folderdata.category_name}}</span></h6>
        </div> 
    </div>
    <div class="row mt-5">
        <div class="col-lg-6">
                ...
                <table class="table align-middle">
                    <thead>
                        <tr> 
                            ...
                            <th class="fw-semibold" width="16%">Date Upload</th>
                            <th class="fw-semibold" width="10%">Type</th>
                            <th class="fw-semibold" width="10%">Owner</th>
                            <th class="fw-semibold" width="10%">Size</th>
                            <th class="fw-semibold" width="10%">Folder</th>
                            <th class="fw-semibold text-center" width="1%" nowrap>Option</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr ng-repeat="files in filtered = (allrecentobj | filter: search) | limitTo:items_per_page:items_per_page*(current_page-1)">
                            <td>{{files.file_name}}</td>
                            <td>{{files.uploaded_at | date: 'MMM dd, yyyy hh:mm a'}}</td>
                            <td>{{files.file_type}}</td>
                            <td>{{files.file_owner}}</td>
                            <td>{{formatSize(files.file_size)? formatSize(files.file_size) : 0}}</td>  
                            <td>{{files.owner_id != userinfo.user_id? 'Shared File' : files.category_name ? files.category_name : 'My Files'}}</td> 
