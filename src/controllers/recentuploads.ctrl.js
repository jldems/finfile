angular
  ...
  .controller(
    ...
    function ($scope, $state, $filter, $rest, $http, $decrypt, $uibModal, SweetAlert2) {
      document.title = "Recent Uploads | FinFile";
          ...
          function success(res) {  
            if(res.data){ 
              ...
              let files = res.data.map(function (file) {
                file.uploaded_at = new Date(file.uploaded_at.replace(" ", "T"));
                return file;
              });
      ...
      $scope.merge_recent_lists = function () {
        ...
        let recent = Array.isArray($scope.recentobj) ? $scope.recentobj : [];
        let shared = Array.isArray($scope.sharedrecentobj) ? $scope.sharedrecentobj : [];

        $scope.allrecentobj = recent.concat(shared).sort(function (a, b) {
